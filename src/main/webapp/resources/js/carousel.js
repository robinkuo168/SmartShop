...
$('body').on('click', '[id^="btn_edt"]', function(e) {
    ...
    var id = $('.id', row).html(),
	    photoName =$('.photoName', row).html(),
	    googleDrive =$('.googleDrive', row).html(),
	    seq =$('.seq', row).html();
    
    var data = set_data(id, photoName, googleDrive, seq);
...
$('body').on('click', '[id^="btn_del"]', function(e) {
    ...
    var data = set_data(id, "", googleDrive);
    
    data_to_dialog('#dlg_delete', data);
	
    $('#dlg_delete > div.content').html(`是否刪除此筆資料，Google Drive：${data.googleDrive}?`);
    $('#dlg_delete').dialog({
	    title: "刪除資料",
	    modal: true,
	    width: 400,
...
function set_data(id, photoName, googleDrive, seq) {
	return data = {
		id: id,
		photoName: photoName,
